...
// Push a property change from the mpv event thread back to Kotlin (MPVLib).
...
// Blocking pump. Runs on event_thread. Wakes on mpv events or g_event_thread_request_exit.
void *event_thread(void *arg)
{
    ...
    while (1) {
        ...
        case MPV_EVENT_END_FILE: {
            mpv_event_end_file *ef = (mpv_event_end_file*)mp_event->data;
            env->CallStaticVoidMethod(mpv_MPVLib, mpv_MPVLib_eventEndFile, (jint) ef->reason);
            break;
        }
        default:
